...
public class Sphere extends BaseObject {
    ...
    public Sphere(float x, float y, float z, float r, int stacks, int slices) {
        this.position = new Point3D(x, y, z);
        vertices = new Point3D[(stacks + 1) * (slices + 1)];
        ...
        for (int i = 0; i < stacks; i++) {
            for (int j = 0; j < slices; j++) {
                ...
                indices[iCount++] = first; indices[iCount++] = second; indices[iCount++] = first + 1;
                indices[iCount++] = second; indices[iCount++] = second + 1; indices[iCount++] = first + 1;
            }
        }
    }
